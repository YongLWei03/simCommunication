...
CString defUnitType[]={"保留","变压器差动保护","变压器后备保护","中压进线保护","中压馈线保护","电容器保护","所用变保护","备自投","综合测控",""};
//                        0          1             2                  3                4             5           6          7        8 
...
/*
	      if(g_Unit[unitno].UnitPT==0)
        {
          switch(g_Unit[unitno].UnitType)
          {
            ...
            case 7://综合测控
            case 8://备自投
              g_Unit[unitno].UnitPT = (float)1;
              g_Unit[unitno].UnitCT = (float)1;
              break;
            case 2://变压器高后备保护
              g_Unit[unitno].UnitPT = (float)110 / (float)100;
              break;
            case 3://变压器低后备保护
            case 4://线路保护
            case 5://电容器保护
              g_Unit[unitno].UnitPT = (float)27.5 / (float)100;
              break;
            default:
              g_Unit[unitno].UnitPT = (float)1;
              g_Unit[unitno].UnitCT = (float)1;
	      ...
	      if(g_Unit[unitno].UnitCT==0)
          g_Unit[unitno].UnitCT = 200;
*/
        {
          for(int vargroupno=0;vargroupno<VarGroupCount;vargroupno++)
          {
...
BOOL WINAPI ClearSceneOperate(TagSceneOperateStruct *pSceneOperate)
{
  ...
  pSceneOperate->InlineVoltage.Ua1 = (float)atof(buf);
  pSceneOperate->InlineVoltage.Ub1 = (float)atof(buf);
  pSceneOperate->InlineVoltage.Uc1 = (float)atof(buf);
  pSceneOperate->InlineVoltage.Ua2 = (float)atof(buf);
  pSceneOperate->InlineVoltage.Ub2 = (float)atof(buf);
  pSceneOperate->InlineVoltage.Uc2 = (float)atof(buf);
  ...
  g_Unit[g_UnitID1THi].VarGroup[0].Var[15].Value = (float)pSceneOperate->PTVoltage.T1Gear;
  g_Unit[g_UnitID2THi].VarGroup[0].Var[15].Value = (float)pSceneOperate->PTVoltage.T2Gear;
  g_Unit[g_UnitID1THi].VarGroup[0].Var[16].Value = (float)pSceneOperate->PTVoltage.T1Temp1;
  g_Unit[g_UnitID2THi].VarGroup[0].Var[16].Value = (float)pSceneOperate->PTVoltage.T2Temp1;
  g_Unit[g_UnitID1THi].VarGroup[0].Var[17].Value = (float)pSceneOperate->PTVoltage.T1Temp2;
  g_Unit[g_UnitID2THi].VarGroup[0].Var[17].Value = (float)pSceneOperate->PTVoltage.T2Temp2;
